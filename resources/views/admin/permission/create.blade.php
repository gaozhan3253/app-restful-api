...
<article class="cl pd-20">
    <form action="{{url('/admin/permission')}}" method="post" class="form form-horizontal" id="form-admin-role-add">
    {{csrf_field()}}
        <div class="row cl">
               ...
               <span class="select-box" style="width:150px;">
				<select class="select valid" name="cid" size="1" aria-required="true" aria-invalid="false">
                    ...
                    @foreach($permissions as $permission)
                        <option value="{{$permission->id}}">|@for($i =$permission->key;$i>1;$i--) ---- @endfor{{$permission->label}}</option>
                    @endforeach
				</select>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="" placeholder="" id="label" name="label" datatype="*4-16" nullmsg="权限名称不能为空">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"> 字段名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="" placeholder="" id="name" name="name" datatype="*4-16" nullmsg="字段名称不能为空">
            </div>
        </div>
        <div class="row cl">
