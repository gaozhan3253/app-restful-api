...
<article class="cl pd-20">
    <form action="{{url('/admin/category/'.$result->id)}}" method="post" class="form form-horizontal" id="form-admin-role-add">
    {{csrf_field()}}
               ...
               <span class="select-box" style="width:150px;">
				<select class="select valid" name="cid" size="1" aria-required="true" aria-invalid="false">
                    ...
                    @foreach($categorys as $category)
                        <option @if($category->id == $result->cid) selected @endif value="{{$category->id}}">|@for($i =$category->key;$i>1;$i--) ---- @endfor{{$category->name}}</option>
                    @endforeach
				</select>
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"  placeholder="" id="name"  value="{{$result->name}}"  name="name" datatype="*4-16" nullmsg="分类名称不能为空">
            </div>
        </div>
        <div class="row cl">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text"   id="sort" name="sort" datatype="*4-16" value="{{$result->sort}}">
            </div>
        </div>
        <div class="row cl">
