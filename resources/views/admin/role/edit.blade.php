...
<article class="cl pd-20">
    <form action="{{url('/admin/role/'.$role->id)}}" method="post" class="form form-horizontal" id="form-admin-role-add">
        {{csrf_field()}}
                    ...
                    <dt>
                        <label>
                            <input type="checkbox" value="{{$permission->id}}"
                                   ...
                                   checked
                                   @endif
                                   name="permission[]" id="user-Character-0">
                                ...
                                @if($permission1->cid == $permission->id)
                                    <dd>
                                        <dl class="cl permission-list2">
                                            <dt>
                                                <label class="">
                                                    <input type="checkbox" value="{{$permission1->id}}"
                                                           @if($role->hasPermission($permission1->name))
                                                           checked
                                                           @endif
                                                           name="permission[]" id="user-Character-0-0">
                                            ...
                                            <dd>
                                                ...
                                                <label class="">
                                                    <input type="checkbox" value="{{$permission2->id}}"
                                                           @if($role->hasPermission($permission2->name))
                                                           checked
                                                           @endif
